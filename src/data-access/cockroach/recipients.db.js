function makeRecipientsDb({ cockroach }) {
  return { insertRecipientsIntoDb };
  // const res = await cockroach.query("SELECT $1::text as message", [
  //   "Hello world!",
  // ]);
  async function insertRecipientsIntoDb({
    databaseName,
    ...
    try {
      ...
      let result = await cockroach.query(
        `INSERT INTO ${databaseName}.recipients (recipient_email_address,email_id,type) values ($1,$2,$3)`,
        [recipientEmailAddress, emailId, type]
      );
      return result;
    } catch (err) {
      throw err;
...
module.exports = makeRecipientsDb;
